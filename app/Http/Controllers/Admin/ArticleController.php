...
class ArticleController extends Controller
{
      ...
      public function store(Request $request) // Laravel 的依赖注入系统会自动初始化我们需要的 Request 类
    {
          ...
          $article->title = $this->clean($request->get('title')); // 将 POST 提交过了的 title 字段的值赋给 article 的 title 属性
          $article->body = $this->clean($request->get('body')); // 同上
          ...
          // 将数据保存到数据库，通过判断保存结果，控制页面进行不同跳转
            ...
            public function update(Request $request, $id)
            {
                ...
                $article->title = $this->clean($request->get('title'));
                $article->body = $this->clean($request->get('body'));
